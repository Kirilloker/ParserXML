...
namespace CodeProviders
{
    class CompileSample
    {
        ...
        public void Build(string name, string sourse, string resPath)
        {
            CodeDomProvider compiler = CodeDomProvider.CreateProvider("CSharp");
            var settings = new CompilerParameters();
            settings.GenerateExecutable = true;
            settings.OutputAssembly = name;
            for (int i = 0; i < ListRefrencedAssemblies.Length; i++)
            {
                settings.ReferencedAssemblies.Add(ListRefrencedAssemblies[i]);
            }
            settings.CompilerOptions = "/target:winexe";
            settings.TreatWarningsAsErrors = false;
            settings.EmbeddedResources.Add(resPath);
            CompilerResults result = compiler.CompileAssemblyFromSource(settings, sourse);
